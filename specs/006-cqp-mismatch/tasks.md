...
- [ ] T001 Build baseline: run the project's existing CMake configure + build on the current branch (`006-cqp-mismatch`) and confirm a clean build (no warnings beyond the project's existing baseline). Record commit SHA so the Polish phase can diff against it.
