...
void SlotManager::shutdown()
{
	...
	for (auto &kv : shared_) {
		blog(LOG_ERROR, "[multi-scene-rec] leaked shared encoder context for group '%s' at shutdown",
		     kv.first.c_str());
	}
	shared_.clear();
}
...
EffectiveRC SlotManager::effective_rate_control(const SceneSlot::Config &c) const
{
	...
	//     leaf shared_mtx_ briefly, never nested with mtx_ above.
	{
		...
		if (it != shared_.end() && it->second && it->second->encoder_fallback_) {
			eff.mode = "CBR";
			eff.value = rc_util::is_bitrate_based(owner.rate_control) ? owner.rc_value : 6000;
			eff.fallback = true;
		}
	}
	return eff;
...
SharedEncoder *SlotManager::acquire_shared_encoder(const std::string &group_key, const SceneSlot::Config &owner_cfg)
{
	...
	if (it == shared_.end()) {
		...
		if (!ctx->build(owner_cfg)) {
			blog(LOG_ERROR, "[multi-scene-rec] failed to build shared encoder for group '%s'",
			     group_key.c_str());
	...
	if (!ref) {
		blog(LOG_ERROR, "[multi-scene-rec] shared encoder for group '%s' is being destroyed",
		     group_key.c_str());
		if (ctx->use_count_ == 0)
...
void SlotManager::release_shared_encoder(const std::string &group_key)
{
	...
	auto it = shared_.find(group_key);
	if (it == shared_.end())
		return; // defensive: never built / already gone
	SharedEncoder *ctx = it->second.get();
...
static SceneSlot::Config slot_from_data(obs_data_t *d, std::unordered_map<std::string, obs_properties_t *> &props_cache)
{
	SceneSlot::Config c;
	...
	if (c.video_encoder_id.empty())
		c.video_encoder_id = "obs_x264";
	if (c.audio_encoder_id.empty())
		c.audio_encoder_id = "ffmpeg_aac";
	if (c.rate_control.empty())
		c.rate_control = "CBR";
	if (c.rc_value == 0) {
	...
	if (c.height == 0)
		c.height = 1080;
	if (c.fps_num == 0 || c.fps_num > 240000)
		c.fps_num = 60;
	if (c.fps_den == 0 || c.fps_den > 1001)
		c.fps_den = 1;
	if (c.container.empty())
		c.container = "mp4";
	if (c.audio_bitrate == 0)
		c.audio_bitrate = 160;
	if (c.replay_seconds == 0)
		c.replay_seconds = 30;
			...
			if (rc_prop && obs_property_get_type(rc_prop) == OBS_PROPERTY_LIST) {
				...
				if (!in_list && !first_mode.empty()) {
					...
					blog(LOG_WARNING,
					     ...
					     c.name.c_str(), original_rc.c_str(), c.video_encoder_id.c_str(),
					     c.rate_control.c_str());
				}
			}
			...
			if (!rc_util::is_lossless(c.rate_control)) {
				...
				if (range_p && obs_property_get_type(range_p) == OBS_PROPERTY_INT) {
					...
					if ((int)c.rc_value < rmin || (int)c.rc_value > rmax) {
						...
						c.rc_value = (int)c.rc_value < rmin ? (uint32_t)rmin : (uint32_t)rmax;
						blog(LOG_WARNING,
	...
	if (c.keyframe_interval_sec == 0)
		c.keyframe_interval_sec = 2;
	...
	c.psycho_aq = obs_data_has_user_value(d, "psycho_aq") ? obs_data_get_bool(d, "psycho_aq") : true;
	...
	c.cabac = obs_data_has_user_value(d, "cabac") ? obs_data_get_bool(d, "cabac") : true;

	// mbtree: same reasoning as cabac.
	c.mbtree = obs_data_has_user_value(d, "mbtree") ? obs_data_get_bool(d, "mbtree") : true;
	...
	c.b_frames = obs_data_has_user_value(d, "b_frames") ? (int)obs_data_get_int(d, "b_frames") : -1;
	c.gpu_index = obs_data_has_user_value(d, "gpu_index") ? (int)obs_data_get_int(d, "gpu_index") : -1;
	c.max_qp = obs_data_has_user_value(d, "max_qp") ? (int)obs_data_get_int(d, "max_qp") : -1;
	c.min_qp = obs_data_has_user_value(d, "min_qp") ? (int)obs_data_get_int(d, "min_qp") : -1;
	c.aq_mode = obs_data_has_user_value(d, "aq_mode") ? (int)obs_data_get_int(d, "aq_mode") : -1;
	c.qsv_async_depth = obs_data_has_user_value(d, "qsv_async_depth") ? (int)obs_data_get_int(d, "qsv_async_depth")
									  : -1;
	c.vt_frames_before_start = obs_data_has_user_value(d, "vt_frames_before_start")
					   ? (int)obs_data_get_int(d, "vt_frames_before_start")
					   : -1;
...
void SlotManager::load_from(obs_data_t *save_data)
{
...

	{
		...
		for (auto &consumer : slots_) {
			...
			if (cc.shared_encoder_slot_id.empty())
				continue;
			bool resolved = false;
...
void SlotManager::save_cb(obs_data_t *save_data, bool saving, void *ptr)
{
	...
	if (!self)
		return;
	if (saving)
		self->save_to(save_data);
	else
		self->load_from(save_data);
}
