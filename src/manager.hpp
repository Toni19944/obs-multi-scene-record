...
struct EffectiveRC {
	...
	// Bitrate kbps or quality level (undefined when rc_util::is_lossless(mode)).
	uint32_t value = 0;
...
public:
	...
	bool config_by_slot_id(const std::string &slot_id, SceneSlot::Config &out) const;
	...
	SharedEncoder *acquire_shared_encoder(const std::string &group_key, const SceneSlot::Config &owner_cfg);
