...
public:
	...
	void set_pending_hotkey_bindings(obs_data_array_t *rec, obs_data_array_t *rep);

	static void on_record_hotkey(void *data, obs_hotkey_id id, obs_hotkey_t *hotkey, bool pressed);
	static void on_save_hotkey(void *data, obs_hotkey_id id, obs_hotkey_t *hotkey, bool pressed);
...
private:
	...
	bool setup_encoders(const Config &cfg, std::vector<obs_encoder_t *> &aencs, std::vector<int> &tracks);
	...
	bool setup_outputs(const Config &cfg, const struct EffectiveRC &eff, obs_encoder_t *venc,
			   const std::vector<obs_encoder_t *> &aencs, obs_output_t **out_rec, obs_output_t **out_replay,
			   ReplaySizing *sizing);
...
uint64_t resolve_max_size_mb(const SceneSlot::Config &cfg, const struct EffectiveRC &eff, bool *out_was_clamped,
			     uint64_t *out_requested_mb, uint64_t *out_auto_mb = nullptr,
